...
class CornersProblem(search.SearchProblem):
    ...
    def getStartState(self):
        ...
        """
        "*** YOUR CODE HERE ***"
        corners = tuple(i for i in self.corners)
        ...
        for action in [Directions.NORTH, Directions.SOUTH, Directions.EAST, Directions.WEST]:
            ...
            if self.walls[x][y] != True:
                if (x, y) in self.corners:
                    corners = tuple(
                        (i for i in state[-1] if i != (x, y)))
                    successors.append([((x, y), corners), dir, 1])
                else:
                    successors.append([((x, y), state[-1]), dir, 1])
...
def cornersHeuristic(state: Any, problem: CornersProblem):
    ...
    if my_corners == ():
        return 0
    for xy2 in my_corners:
        res.append(abs(xy1[0][0] - xy2[0]) + abs(xy1[0][1] - xy2[1]))
...
def foodHeuristic(state: Tuple[Tuple, List[List]], problem: FoodSearchProblem):
    ...
    xy1 = state
    food_coords = []
    for y, i in enumerate(foodGrid):
        for x, j in enumerate(i):
            if j:
                food_coords.append((y, x))

    res = []
    if food_coords == []:
        return 0
    for xy2 in food_coords:
        res.append(mazeDistance(xy1[0], xy2, problem.startingGameState))
